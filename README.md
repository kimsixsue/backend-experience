...
실무에서 경험한 PHP 웹서비스 운영·유지보수 흐름을 바탕으로, 회사 코드나 데이터를 사용하지 않고 대학교·공공기관 부속 홈페이지 관리 시스템을 가정해 구현하고 있습니다.

현재는 요구사항정의서, DB 설계 초안, 화면목록, 개발기록을 정리한 뒤 Docker Compose 기반 개발환경, PDO 기반 DB 연결, 관리자 로그인/로그아웃, 세션 기반 로그인 처리, 역할 기반 접근 제한 기초 기능을 구현했습니다.
